...
public partial class app_communicators_profileedit : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            XmlNode XmlResult = oDecode.LoadUser(iPublicationID, "");
            ...
            {
                divProfileImage.Style.Add("background-image", "url('../images/profileimages/" + iPublicationID + "." + sImageFormat + "')");
            }
            else
            {
            ...
            string s = Convert.ToString(oDecode.LoadUserArticles(iPublicationID).InnerXml);
            doc.LoadXml(s);
    ...
    protected void btnBack_Click(object sender, EventArgs e)
    {
        try
        {
            var oGeneralFunctions = new GeneralFunctions();
            oGeneralFunctions.UserSessionTrail(Convert.ToInt32(Session["iUserID"].ToString()), HttpContext.Current.Session.SessionID.ToString(), Request.RawUrl.ToString());
            Response.Redirect("~/app/admin/communicators.aspx", true);
        }
        catch (Exception ex)
        {
            var oGeneralFunctions = new GeneralFunctions();
    ...
    protected void btnSaveUserArticle_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int iPublicationID = Convert.ToInt32(Request.QueryString["pid"].ToString());
            var oDecode = new za.co.idecode.test.idecode();
            bool bSaveUserArticle = oDecode.NewUserArticle(iPublicationID, txtArticleTitle.Text, txtArticleMediaOutlet.Text, txtDescription.Text, txtArticleURL.Text, Convert.ToDateTime(txtArticleDatePublished.Text), "", "", Convert.ToBoolean(1), txtArticleImageURL.Text);
            dsUserArticles.DataBind();
